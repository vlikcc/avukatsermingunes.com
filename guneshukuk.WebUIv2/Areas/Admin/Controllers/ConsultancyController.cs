...
namespace guneshukuk.WebUIv2.Areas.Admin.Controllers
{
    ...
    public class ConsultancyController(IHttpClientFactory httpClientFactory) : Controller
	{
		...
		public async Task<IActionResult> CreateConsultancy(CreateConsultancyDto createConsultancyDto)
		{
			...
			var responseMessage = await httpClient.PostAsync("https://guneshukukwebapi.azurewebsites.net/api/Consultancy/CreateConsultancy", content);
		...
		{
			HttpClient httpClient = httpClientFactory.CreateClient();
			var response = await httpClient.DeleteAsync($"https://guneshukukwebapi.azurewebsites.net/api/Consultancy/DeleteConsultancy/{id}");
		...
		{
			HttpClient httpclient = httpClientFactory.CreateClient();
			var responseMessage = await httpclient.GetAsync($"https://guneshukukwebapi.azurewebsites.net/api/Consultancy/GetConsultancyById?Id={Id}");
		...
		public async Task<IActionResult> UpdateConsultancy(UpdateConsultancyDto updateConsultancyDto)
		{
			...
			StringContent content = new StringContent(jsonData, Encoding.UTF8, "application/json");
			var responseMessage = await httpClient.PutAsync("https://guneshukukwebapi.azurewebsites.net/api/Consultancy/UpdateConsultancy", content);
			if (responseMessage.IsSuccessStatusCode)
			{
				return RedirectToAction("ListConsultancies");
			}
